...
const Upload = () => {
  ...
  const addToUploadedArray = () => {
    const storedFiles = JSON.parse(localStorage.getItem("pdfs") || "[]"); // Get existing files array
    const updatedFiles = [...storedFiles, preview]; // Add new file
    localStorage.setItem("pdfs", JSON.stringify(updatedFiles)); // Save back to localStorage
  };
      ...
      {preview.file_url ? (
        ...
        >
          <Image
            ...
            width={20}
          />
          {preview?.file_name && <p>{preview.file_name}</p>}
          <input {...getInputProps()} />
        </div>
      ) : (
        <div
